...
//        });
        ...
        adapterWrapper.setSpanSizeLookup(new LRecyclerViewAdapter.SpanSizeLookup() {
            ...
            public int getSpanSize(GridLayoutManager gridLayoutManager, int position) {
               //先获取ItemType
                ...
                if (position<adapterWrapper.getHeaderViewsCount()||position>(adapterWrapper.getHeaderViewsCount()+adapterWrapper.getItemCount())){
                    //f当前位置的Item是header，占用列数和spanCount一样。
                    return ((GridLayoutManager)layoutManager).getSpanCount();
                }
    ...
    private void fetchData() {
        ...
        //数据添加到ArrayList当中，list里面使用的是嵌套类型。
