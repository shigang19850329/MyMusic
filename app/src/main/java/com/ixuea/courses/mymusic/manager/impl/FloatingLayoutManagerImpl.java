...
import com.ixuea.courses.mymusic.activity.MainActivity;
...
public class FloatingLayoutManagerImpl implements FloatingLayoutManager, FloatingLinearLayoutView.OnTouchDragListener {
    ...
    /**
     * 这里有一个bug，当打开其他应用再点返回键的时候，没有效果，返回按钮还是回到这个WindowManager里面
     */
    ...
    private void startMusicPlayActivity() {
        ...
        //启动MainActivity，在MainActivity中判断Action然后启动播放界面。
    ...
    private void removeFloatingLayout() {
        //在windowManager中删除floatingLayout，而不是销毁它，下一次再使用的时候直接添加回来就可以了。
